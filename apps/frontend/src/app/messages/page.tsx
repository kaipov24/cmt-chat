...
import { DirectMessagesPanel } from '@/components/direct-messages-panel';
...
export default function MessagesPage() {
  return (
    <main className="min-h-screen bg-[#f4f7fa] text-slate-950">
      ...
      <div className="mx-auto grid max-w-[1200px] gap-5 px-4 py-6 sm:px-6">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Messages</p>
          <h1 className="mt-1 text-3xl font-semibold text-slate-950">Direct messages</h1>
          <p className="mt-2 max-w-2xl text-sm text-slate-600">
            Chat privately with another member.
          </p>
        </div>

        <Suspense fallback={<div className="rounded-lg border border-slate-200 bg-white p-5 text-sm">Loading messages...</div>}>
          <DirectMessagesPanel />
        </Suspense>
      </div>
    </main>
